		...
		if (H.original_job)
			if (istype(item, /obj/item/weapon/reagent_containers/food/drinks/bottle))
				...
				if (B.rag && B.rag.on_fire)
					var/nothrow = FALSE
					if (map && !map.faction1_can_cross_blocks() && list(REDLINE, CIVILIAN, PIRATES, INDIANS, GREEK, ARAB, VIETNAMESE, CHINESE).Find(H.original_job.base_type_flag()))
						nothrow = TRUE
					else if (map && !map.faction2_can_cross_blocks() && list(REICH, BRITISH, FRENCH, PORTUGUESE, SPANISH, DUTCH, ROMAN, JAPANESE, RUSSIAN, GERMAN, AMERICAN).Find(H.original_job.base_type_flag()))
						nothrow = TRUE
					if (nothrow)
